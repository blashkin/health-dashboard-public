...
const STAGE_ROWS=[['sleep_deep_hours','глубокий','deep','Глубокий — медленный сон: пульс и дыхание самые редкие, тело восстанавливается сильнее всего. Обычно первая половина ночи.'],['sleep_core_hours','лёгкий','core','Лёгкий — всё остальное время сна, между глубоким и REM; у Apple эта стадия называется Core. Обычно его больше половины ночи.'],['sleep_rem_hours','REM','rem','REM — rapid eye movement, «быстрые движения глаз»: стадия, в которой чаще всего снятся сны, мозг активен почти как наяву. Обычно ближе к утру.'],['sleep_unspecified_hours','без стадии','unspec','Без стадии — часы записали «спал», но не разметили стадию: старые watchOS до версии 9 или стороннее приложение.']];
...
function chart(rows,label,unit,opts){
 ...
 return chartLegend(label,unit)+
  `<div class="chart-wrap"><svg class="chart" viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="График: ${esc(label)}"><clipPath id="clip-${esc(key||'chart')}"><rect x="${p.l}" y="${p.t}" width="${plot}" height="${h-p.t-p.b}"/></clipPath><g clip-path="url(#clip-${esc(key||'chart')})">${bands}</g>${ticks}${body}${labs}${axisY}${axisX}</svg></div>`+
  chartToggle(key,kind,opts.metric,annual)}
// Текст подсказки у точки: месяц, значение и, если известно, дни с записью.
function tipText(row,v,unit){let days=row.observed_days!=null&&row.calendar_days!=null&&n(row.calendar_days)>1?`\nдней с записью: ${fmt(n(row.observed_days),0)} из ${fmt(n(row.calendar_days),0)}`:row.observed_days!=null?`\nзаписей: ${fmt(n(row.observed_days),0)}`:'';return `${monthName(row.month)} · ${fmt(v,Number.isInteger(v)?0:v>=100?1:2)} ${unit}${days}`}
...
function sleepStagesBlock(){let d=sleepStages();if(!d)return '';let W=1100,L=48,R=20,T=16,B=60,H=320-T-B,bw=(W-L-R)/d.months.length,svg='';
 ...
 <p class="legend">${STAGE_ROWS.map(([_,label,cls,help])=>`<span class="help" tabindex="0" data-help="${esc(help)}" aria-label="${esc(label)}. ${esc(help)}"><i class="sw stage-${cls}"></i>${esc(label)}</span>`).join('')}</p>
